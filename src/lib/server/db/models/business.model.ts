...
export const BusinessTable = pgTable(
  ...
  (table) => [index("idx_business_org_id").on(table["org_id"])],
);

export const business_relations = relations(BusinessTable, ({ one }) => ({
  organization: one(OrganizationTable, {
    fields: [BusinessTable.org_id],
...
export const BusinessSchema = {
  ...
  update: createUpdateSchema(BusinessTable, refinements).pick(pick).extend({ id: z.uuid() }),
};
