  ...
  constructor(
    ...
    $effect(() => {
      ...
      if (
        ...
        const { warning, suggestions } = this.root.strength.feedback;

        const issues = [warning, ...suggestions.map((s) => "- " + s)].flatMap(
          (i) => (i ? [i] : []),
        );

        if (issues.length === 0) {
          issues.push("Please choose a stronger password");
        }

        this.opts.ref.current?.setCustomValidity(issues.join("\n"));
      } else {
